...
The notebook is intentionally set up to be customizable based on one's needs, with commented instructions at crucial points. For example, the query for filtering the data will be saved to a CSV may be be modified using terminology from Scryfall's query language.

This query: `'set:ktk type:creature order:cmc'` will return all creatures from the set *Khans of Tarkir*, ordered by cmc.
